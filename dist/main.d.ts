...
export interface WAExtendedTextMessage {
    ...
    font?: "SANS_SERIF" | "SERIF" | "NORICAN_REGULAR" | "BRYDAN_WRITE" | "BEBASNEUE_REGULAR" | "OSWALD_HEAVY" | null;
...
export interface WAProtocolMessage {
    ...
    type?: "REVOKE" | "EPHEMERAL_SETTING" | "EPHEMERAL_SYNC_RESPONSE" | "HISTORY_SYNC_NOTIFICATION" | null;
...
export interface WAHistorySyncNotification {
    ...
    syncType?: "INITIAL_BOOTSTRAP" | "INITIAL_STATUS_V3" | "FULL" | "RECENT" | null;
    chunkOrder: number;
}
...
export interface WAWebMessage {
    ...
    status?: "ERROR" | "PENDING" | "SERVER_ACK" | "DELIVERY_ACK" | "READ" | "PLAYED" | null;
    participant?: string | null;
    author?: string;
}
...
export default class WhatsApp {
    ...
    private messageStubListeners;
    private noNetworkListeners;
    private loggedOutListeners;
    private eventListeners;
    private readyListeners;
    private qrCodeListeners;
    constructor(parameters?: {});
    on(event: "message" | "ready" | "stubMessage" | "qrCode" | "noNetwork" | "loggedOut", cb: (() => void) | ((msg: WAWebMessage, description: string) => void) | ((msg: WAStubMessage) => void)): void;
    ...
    private keepAlive;
    disconnect(): void;
    private sendSocketAsync;
    private onMessage;
    private decryptMessage;
    private sendAdminTest;
    private sendProto;
    private sendMessage;
    private getGroupMetadata;
    getGroupParticipants(remoteJid: string): Promise<{
        id: string;
        isAdmin: boolean;
        isSuperAdmin: boolean;
    }[]>;
    getGroupSubject(remoteJid: string): Promise<string>;
    setGroupPhoto(image: Buffer, remoteJid: string): Promise<{
        id: string;
        content: WAMessageNode;
    }>;
    sendTextMessage(text: string, remoteJid: string, mentionedJid?: WAContextInfo["mentionedJid"]): Promise<{
        id: string;
        content: WAMessage;
    }>;
    private sendQuotedMessage;
    sendQuotedTextMessage(text: string, remoteJid: string, quotedAuthorJid: string, quotedMsg: WAMessage, quotedMsgId: string, mentionedJid?: WAContextInfo["mentionedJid"]): Promise<{
        id: string;
        content: WAMessage;
    }>;
    sendQuotedMediaMessage(file: Buffer, mimetype: string, msgType: "image" | "sticker" | "video" | "audio" | "document", remoteJid: string, quotedAuthorJid: string, quotedMsg: WAMessage, quotedMsgId: string, caption?: string | undefined, duration?: number | undefined, isGif?: boolean, mentionedJid?: WAContextInfo["mentionedJid"]): Promise<{
        id: string;
        content: WAMessage;
    }>;
    sendQuotedContactVCard(vcard: string, remoteJid: string, quotedAuthorJid: string, quotedMsg: WAMessage, quotedMsgId: string, mentionedJid?: WAContextInfo["mentionedJid"]): Promise<{
        id: string;
        content: WAMessage;
    }>;
    sendQuotedContact(phoneNumber: string, firstName: string, lastName: string | undefined, remoteJid: string, quotedAuthorJid: string, quotedMsg: WAMessage, quotedMsgId: string, mentionedJid?: WAContextInfo["mentionedJid"]): Promise<void>;
    sendQuotedLocation(latitude: number, longitude: number, remoteJid: string, quotedAuthorJid: string, quotedMsg: WAMessage, quotedMsgId: string, mentionedJid?: WAContextInfo["mentionedJid"]): Promise<{
        id: string;
        content: WAMessage;
    }>;
    private uploadMedia;
    ...
    sendVCardContact(remoteJid: string, vcard: string): Promise<{
        id: string;
        content: WAMessage;
    }>;
    sendContact(remoteJid: string, phoneNumber: string, firstName: string, lastName?: string): Promise<{
        id: string;
        content: WAMessage;
    }>;
    sendLocation(remoteJid: string, latitude: number, longitude: number): Promise<{
        id: string;
        content: WAMessage;
    }>;
    private sendMediaProto;
    private decryptMedia;
    deleteMessage(remoteJid: string, msgId: string): Promise<{
        id: string;
        content: WAMessage;
    }>;
    getProfilePicThumb(jid: string): Promise<{
        id: string;
        content?: Buffer;
        status?: number;
    }>;
